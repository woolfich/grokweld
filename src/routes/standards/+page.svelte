<script lang="ts">
  ...
  async function addStandard() {
    ...
    if (!normalizedArticle || isNaN(timeValue) || timeValue <= 0) {
      alert('Введите корректный артикул и время');
      return;
    }
    ...
    const newStandard = { article: normalizedArticle, time: timeValue, createdAt: new Date() };
    const newId = await db.standards.add(newStandard);
    standards.update(current =>
      [...current, { ...newStandard, id: newId }].sort((a, b) => a.article.localeCompare(b.article))
    );
    article = '';
    time = '';
  }
